...
- **不影响其他模式**：宿主组成、`profiles/*/cordis.patch.yml`、默认 preset 一行没改；
  不选「极压模式」就没有任何影响。
- **改动面小、可回滚**：与标准模式的差异只有八处 `config` 值加一段 persona，卸载就是删一个目录。

**代价**

- **原始内容会丢**：保留窗口（≈50K）之前的逐字内容不再回到上下文，只剩摘要。
- **细节会丢**：超过 4 KB 的非 `read` 结果当场被收口成「头 2KB + 尾 2KB + locator」；被压缩掉的旧结果只剩头 1024 / 尾 512 字符。两种情况全文都在 spill 工件里，可按 locator 回取。
- **`read` 另有独立上限**：单次最多 300 行 / 8 KB / 每行 1000 字符（`read` 不参与 spill），读到上限要按 `offset` 续读。
- **>50KB 的结果要两跳**：宿主先落盘一次、preset 再收口一次（4KB~50KB 只需一跳）；模型要多读一次工件才拿到全文。
...
| ArtifactStore（原始内容可回取） | `spill-policy` | 50000（宿主平面默认，未改） | **preset 内 4000** |
| S6 文件读取上限 | `tool-fs` | 2000 行 / 50 KB / 每行 2000 字符 | **300 行 / 8 KB / 每行 1000 字符** |
...
- **先定位、再窄读**：先用 `grep -n`/`glob`/`wc -l` 定位，再读最小窗口（`offset`/`limit`、`sed -n 'A,Bp'`）；
  单次读目标 ≤ 约 60 行 / 4 KB。答案不在窗口里就**再窄一点重读**，而不是把范围放大——放大才是失败模式。
- **不盲开文件**：`read` 不是侦察工具——从第 1 行开始读「看看这文件里有什么」必然读多。先 `wc -l` 取行数、
  用 `grep -n` 找锚点，再让第一个窗口从关键行开始往后读，单窗 ≤60 行；多个定位窗口胜过一次大读。
- **输出先进笼子再进上下文**：任何命令都不许无界打印；先 `wc -l`/`head -n 20`/`grep -c` 探路，只打印需要的那一段；
  单条命令输出上限约 4 KB。
- **不假装 harness 会兜住**：契约明说「工具结果的只留头尾**只发生在压缩那一刻**」，
  在那之前模型打印的每一个字节都原样留在上下文里（这两条加严就是实测出来的：一次 91 行 `read` 带回 11 KB、
  一次 `sed -n '60,200p'` 带回 5.5 KB，都成了那几步的最大单笔开销）。
- **批量**：互不依赖的调用放在同一步。
